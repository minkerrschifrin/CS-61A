...
def product(n, term):
    ...
    True
    """
    """ Look through textbook for good examples!"""
    total, k = 1, n
    while k >= 1:
...
def pingpong(n):
    ...
    True
    """
    def helper(index, value, increase):
        if index == n:
            return value + increase
        if index % 7 == 0 or num_sevens(index):
            return helper(index + 1, value + increase, - increase)
        else:
            return helper(index + 1, value + increase, increase)
    return helper(1, 0, 1)
...
def count_change(amount):
    ...
    def helper(amount, coin):
        ...
        if amount == 2**coin:
            ...
            return helper(amount, coin + 1) + helper(amount - 2**coin, coin)

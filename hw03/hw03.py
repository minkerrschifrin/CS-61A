...
def balanced(m):
    ...
    False
    """
    #left_side = total_weight(end(left(m))) * length(left(m))
    #right_side = total_weight(end(right(m))) * length(right(m))
...
#left_side == right_side and balanced_left and balanced_right
#Why does assigning them to variables not work!??
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        a = total_weight(m)
        b = totals_tree(end(left(m)))
        c = totals_tree(end(right(m)))
        return tree(a, [b, c])

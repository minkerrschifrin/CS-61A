...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        operator = scheme_eval(first, env) #evaluate first expression
        check_procedure(operator) #check operator
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("too many or too few arguments")
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    else: #if the second expression is True or False
        first_expression = scheme_eval(expressions.first, env)
        if scheme_falsep(first_expression):
            return False
        elif scheme_truep(first_expression):
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        ...
        first_expression = scheme_eval(expressions.first, env, True)
        if scheme_falsep(first_expression):

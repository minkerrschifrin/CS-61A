...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect.real_queen:
                return
            # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        if self.place.ant: #if not an ant that can block path
            if not self.place.ant.blocks_path:
                return False
        return self.place.ant is not None
        #if self.place.ant != self.place.bee or if Ant.blocks_path == False:
         #   return False
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    min_range = 0
    max_range = float('inf')
    #tunnel = range(place.ThrowerAnt, hive)
    ...
    def nearest_bee(self, hive):
        ...
        while place is not hive:
            if place.bees:
                if self.min_range <= moves <= self.max_range:
                    return random_or_none(place.bees)
            place = place.entrance #going through tunnel moving one square
            moves+= 1 #moving one spquare through tunnel
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            self.digesting -= 1
        elif self.place.bees: #bees are in the your self.place
            self.eat_bee(random_or_none(self.place.bees)) #call eat bee on a random bee in your self.place
            self.digesting = self.time_to_digest #restart the digest timer to 3
        # END Problem 6
...
class BodyguardAnt(Ant):
    ...
    '''class Ant.is_container(Ant):
    if Ant and not BodyguardAnt:
        is_container = False
        BodyguardAnt.is_container = True'''
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.real_queen = QueenAnt.instances == 0
        self.double_damage = [self]
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        queens_position = self.place
        if not self.real_queen:
            return self.reduce_armor(self.armor)
        while queens_position:
            ant = queens_position.ant
            if ant:
                if ant not in self.double_damage:
                    self.double_damage.append(ant)
                    ant.damage *= 2
                if ant.is_container and ant.contained_ant and ant.contained_ant not in self.double_damage:
                    self.double_damage.append(ant.contained_ant)
                    ant.contained_ant.damage *= 2
            queens_position = queens_position.exit
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if not self.real_queen:
            return Insect.reduce_armor(self, self.armor)
        if self.armor <= 0:
            if self.real_queen:
                bees_win()
        # END Problem 13
...
def make_scare(action, bee):
    ...
    # BEGIN Problem EC
    is_scared = False
    bee.moves_backwards = False
    def new_action(colony):
        bee.moves_backwards = True
